    ...
    for(int i = 0; i < numNode; i++)
    {
        int j1 = (i+numNode)%numNode; // i
            ...
            Vb[i].EG(0,0) = geometry->CELL[j1].g_b*geometry->CELL[j1].alphB*(Vb[j1].C(0,0)-Vb[j3].C(0,0))/geometry->CELL[j1].l_b
                                   ...
                                   -0.5*p.pressure*(Vb[j2].C(1,0)-Vb[j3].C(1,0));
            ...
            Vb[i].EG(1,0) = geometry->CELL[j1].g_b*geometry->CELL[j1].alphB*(Vb[j1].C(1,0)-Vb[j3].C(1,0))/geometry->CELL[j1].l_b
                                   ...
                                   -0.5*p.pressure*(Vb[j3].C(0,0)-Vb[j2].C(0,0));
    }
...
double System:: energy(void)
{
    ...
    yolkContribution = -p.pressure*geometry->A_Y;
...
tuple<double,double,double,double> System:: calculate_numericalTension_YM(void)
{
    // length-tolerance
    double delta_L = 1e-3;
    ...
    initializeParameter();
    run("noUseFile",false);
    // default-epithelial-height
    System* embryoCopy_1 = new System(constant_PARAMETERS_File,vary_PARAMETERS_File,outputDir);
    double epithelialHeight_theoretical = embryoCopy_1->scaleGeometry(delta_L,default_alphaL);
    ///////////////////
    /// theoretical ///
    ///////////////////
    double L_new  = L + delta_L;
    double theoTension = p.default_alphaA + p.default_alphaB - default_alphaL*numNode*epithelialHeight_theoretical/L;
    double theoYoungModulus = ((2.0*numNode*numNode*default_alphaL)/(L_new*L_new))*(p.cellRefArea - ((3.0*numNode*default_alphaL)/(4.0*p.betaCell*L_new)));
    /////////////////
    /// numerical ///
    /////////////////
    embryoCopy_1->p  = p;
    for(int i = 0; i < numNode; i++)
    {
        embryoCopy_1->geometry->CELL[i].alphA = geometry->CELL[i].alphA;
        embryoCopy_1->geometry->CELL[i].alphB = geometry->CELL[i].alphB;
        embryoCopy_1->lateral_Edg[i].alphL = lateral_Edg[i].alphL;
    }
    embryoCopy_1->run("noUseFile",false);
    double oldEnergy = energy();
    double newEnergy = embryoCopy_1->energy();
    double tension_old = (newEnergy - oldEnergy)/delta_L;
    // tension
    double numTension = tension_old;
    double eqilibriumEpitheliumHeight_new = embryoCopy_1->Va[0].C(1,0) - embryoCopy_1->Vb[0].C(1,0);
    System* embryoCopy_2 = new System(constant_PARAMETERS_File,vary_PARAMETERS_File,outputDir);
    embryoCopy_2->epithelialHeight = eqilibriumEpitheliumHeight_new;
    embryoCopy_2->L = embryoCopy_1->L;
    embryoCopy_2->p  = embryoCopy_1->p;
    for(int i = 0; i < numNode; i++)
    {
        embryoCopy_2->geometry->CELL[i].alphA = embryoCopy_1->geometry->CELL[i].alphA;
        embryoCopy_2->geometry->CELL[i].alphB = embryoCopy_1->geometry->CELL[i].alphB;
        embryoCopy_2->lateral_Edg[i].alphL = embryoCopy_1->lateral_Edg[i].alphL;
    }
    double length = embryoCopy_2->L;
    double epithelialHeight_theo = embryoCopy_2->scaleGeometry(delta_L,default_alphaL);
    embryoCopy_2->run("noUseFile",false);
    oldEnergy = embryoCopy_1->energy();
    newEnergy = embryoCopy_2->energy();
    double tension_new = (newEnergy - oldEnergy)/delta_L;
    // young-modulus
    double numYoungModulus = length*((tension_new - tension_old)/delta_L);
    delete embryoCopy_2;
    embryoCopy_2 = NULL;
    // delete-system
    delete embryoCopy_1;
    ...
    return {theoTension,theoYoungModulus,numTension,numYoungModulus};
}
